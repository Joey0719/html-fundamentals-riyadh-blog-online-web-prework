RSpec.describe "Well-Formed HTML Document" do
  ...
  context 'within <body>' do
      it 'the wrapper class section contains the correct header sizes and text content' do
        ...
        expect(title_box.children.any? {|ch| ch.name == "h1"}).to be == true
        expect(title_box.children.select {|ch| ch.name == "h1"}.first.children.first.text).to be == "Welcome to Riyadh"

        expect(title_box.children.any? {|ch| ch.name == "h2"}).to be == true
        expect(title_box.children.select {|ch| ch.name == "h2"}.first.children.first.text).to be == "Where A Traditional Past Meets A Modern Future"
      ...
      it 'the content section contains the correct header sizes, text content, links and images' do
        ...
        expect(content_section.children.any? {|ch| ch.name == "h3"}).to be == true
        ...
        expect(content_section.children.any? {|ch| ch.name == "p"}).to be == true
        ...
        expect(content_section.children.any? {|ch| ch.name == "img"}).to be == true
        ...
        expect(content_section.children.any? {|ch| ch.name == "a"}).to be == true
